...
\begin{proof}Let $i \le j$ in $T$. We want to show that $P[Y_j \mid \mathcal{F}_i] = Y_i$ almost surely.
...
\begin{proof}Let $i \le j$ in $T$. We want to show that $P[Y_j \mid \mathcal{F}_i] \ge Y_i$ almost surely.
...
\begin{proof}Done in a Mathlib PR for finite measures: \href{https://github.com/leanprover-community/mathlib4/pull/27953}{\#27953}.
\end{proof}
...
\begin{proof}We endow $T$ with the order topology. Let us first prove that the right continuation is nondecreasing. Let $s, t \in T$ such that $s \le t$.
\begin{itemize}
...
\begin{proof}  \uses{def:rightLimitFiltration}
...
\begin{proof}  \uses{lem:rightContDef}
...
\begin{proof}  \uses{lem:rightContIsolated}
...
\begin{proof}  \uses{lem:rightContNeBot}
...
\begin{proof}  \uses{lem:rightContNotIsMax}
...
\begin{proof}  \uses{lem:leRightCont, lem:rightContDef}
...
\begin{proof}  \uses{def:rightContinuous, lem:leRightCont}
...
\begin{proof}  \uses{lem:rightContSelf}
...
\begin{proof}  \uses{def:rightContinuous}
...
\begin{proof}Let $X : T \times \Omega \to E$ be a predictable process, we will show that it is progressively measurable. Namely, fixing $t \in T$, denoting
...
\begin{proof}For $t \le s$, the set in question is empty and thusly, trivially measurable. On the other hand, for $s < t$, measurability follows as
...
\begin{proof}  \uses{lem:predictable_Ioc_prod}
...
\begin{proof}  Clearly $\tau_n \downarrow \tau$ as $n \to \infty$ and so it remains to show that each $\tau_n$ is a stopping time.
  Indeed,
...
\begin{proof}  Immediate as we can take \(\tau_n = \tau\) for all \(n\).
\end{proof}
