...
\begin{proof}  \uses{lem:internalCoveringNumber_le_packingNumber, lem:packingNumber_two_le_externalCoveringNumber, lem:externalCoveringNumber_mono, lem:externalCoveringNumber_le_internalCoveringNumber}
\begin{align*}
...
\begin{proof}  \uses{lem:card_pairSet_le, lem:dist_le_of_mem_pairSet, lem:sup_dist_le_two_mul_sup_dist_pairSet}
...
\begin{proof}It suffices to show that $d_E(X_s, X_t)^p = 0$ almost everywhere, which is in turn implied by $\mathbb{E}[d_E(X_s, X_t)^p] \le M d_t(s, t)^q = 0$.
\end{proof}
